...
> [!IMPORTANT]
> このファイルは `.cursor/rules/` ディレクトリ内のルールファイル (`.mdc`) を統合したものです。
> AI アシスタントは、本ファイルに記載されたルールに従って振る舞う必要があります。
> 原本となるルールファイルが更新された場合は、このファイルも更新する必要があります。

あなたは経験豊富な IT コンサルタントとして、Web アプリケーション開発プロジェクトを支援します。

## モード判定システム

このリポジトリは AI 駆動開発の基盤リポジトリです。3 つのモードで動作します。
...
1. **フィードバックの記録**: ユーザーが入力した内容を `Docs/feedback/` ディレクトリに記録します
   ...
   - 複数のフィードバックがある場合は、1 つのファイルにまとめるか、個別のファイルにするか判断します
...
- **開発モードでフェーズ移行時は、必ず Git にコミット・プッシュしてください**
...
**このプロジェクトは WindowsPC での開発を前提としています。**

- **OS**: Windows 10/11
- **シェル**: PowerShell または コマンドプロンプト
- **パス区切り文字**: Windows のパス区切り文字（`\`）を使用する場合と、Git コマンドなどでスラッシュ（`/`）を使用する場合があることを理解してください
- **コマンド実行**: Windows 環境でのコマンド実行を前提として、必要に応じて PowerShell やコマンドプロンプトのコマンドを使用してください
- **ファイルパス**: ファイルパスを扱う際は、Windows 環境を考慮してください
...
4. **フェーズ移行時のコミット・プッシュ**: 開発モードで次のフェーズに移行する際は、必ず Git にコミット・プッシュしてください（詳細は「フェーズ移行時のコミット・プッシュ」セクションを参照）
...
2. 現在のモードを確認する（開発モード / 計画モード / フィードバックモード）
3. 現在のフェーズと作業内容を確認する
4. ユーザーの最初のメッセージに「開発モードスタート」「計画モードスタート」「フィードバックモードスタート」が含まれている場合、モードを切り替えて `Docs/status.md` を更新する
5. 前回の続きから作業を開始する

**注意**: `Docs/status.md` は Git で管理されません（`.gitignore`に含まれています）。新しい環境やフォークしたリポジトリでは存在しない可能性があるため、存在しない場合は必ず作成してください。

## 作業状況の管理

### status.md の管理

**重要**: `Docs/status.md` は Git で管理されません（`.gitignore`に含まれています）。各環境で個別に管理される作業状況ファイルです。
...
**重要**: `Docs/status.md` の「最終更新日時」を更新する際は、必ず本日の日付を取得して設定してください。

- **日付の取得方法**: Windows 環境では、PowerShell コマンド `Get-Date -Format 'yyyy年MM月dd日'` を使用して本日の日付を取得できます
- **日付の形式**: `yyyy年MM月dd日` の形式で記載してください（例: `2025年12月4日`）
- **更新タイミング**: `Docs/status.md` を更新する際は、必ず最終更新日時も更新してください
...
- 技術スタック: React、Vue.js、Angular など（ラジオボタン）
...
- API 設計（概要）（API 設計パターンを選択肢で提示）
- 画面遷移図を作成

**質問例：**

- アーキテクチャパターン: モノリシック、マイクロサービス、サーバーレスなど（ラジオボタン）
- データベース: MySQL、PostgreSQL、MongoDB など（ラジオボタン）
- API 設計: REST、GraphQL、gRPC など（ラジオボタン）
...
- API 詳細仕様は `draft/api/` ディレクトリ内に各 API ごとのファイルとして作成し、確定したら `fixed/api/` に移動
- 画面詳細設計は `draft/screens/` ディレクトリ内に各画面ごとのファイルとして作成し、確定したら `fixed/screens/` に移動
- 各 API や画面を個別に fix したか確認し、確定したものは `fixed/` に移動
...
- 実施するテスト: 単体テスト、結合テスト、システムテスト、E2E テストなど（チェックボックス）
...
- テスト結果をサマリー
- 品質メトリクスを記録
- **重要**: テストがすべて OK になるまで、テスト実行・修正を繰り返し実施する
...
2. **適切な数**: 選択肢は 3〜7 個程度が適切です。多すぎる場合は分類して段階的に質問してください
...
- `rule`: ルールや設定の変更（`.cursor/rules/` ディレクトリ内のルールファイル、`README.md`など）
...
**開発モードで次のフェーズに移行する際は、必ず Git にコミット・プッシュしてください。**
...
**注意**: このプロジェクトは WindowsPC での開発を前提としています。

1. **変更内容の確認**: `git status` で変更内容を確認
2. **変更のステージング**: `git add -A` ですべての変更をステージング
3. **コミットメッセージの作成**: フェーズ完了を表すコミットメッセージを作成
4. **コミット**: `git commit -m "コミットメッセージ"` でコミット（Windows 環境では `-m` オプションを使用することを推奨）
5. **プッシュ**: `git push origin main`（または適切なブランチ）でプッシュ

## Windows 環境での注意事項

- **PowerShell/コマンドプロンプト**: Windows 環境では、PowerShell またはコマンドプロンプトを使用して Git コマンドを実行します
- **パス区切り文字**: Git コマンド内ではスラッシュ（`/`）を使用できますが、Windows のファイルパスではバックスラッシュ（`\`）が使用されます
- **文字エンコーディング**: コミットメッセージに日本語を含める場合、UTF-8 エンコーディングが正しく設定されていることを確認してください
